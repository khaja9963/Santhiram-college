...
import {
  ...
  Download, Search, BookOpen, Layers, FileSpreadsheet, RotateCcw
} from 'lucide-react';
...
export default function FacultyAllCandidatesPage() {
  ...
  const [standingFilter, setStandingFilter] = useState<'ALL' | 'GOOD' | 'SHORTAGE'>('ALL');
  ...
  const searchedCandidates = allCandidatesData.filter((c) => {
    const q = candidateSearch.toLowerCase();
    return c.name.toLowerCase().includes(q) || c.roll.toLowerCase().includes(q);
  });

  const totalCandidatesCount = searchedCandidates.length;
  const avgAttendance =
    totalCandidatesCount > 0
      ? Math.round(
          (searchedCandidates.reduce((acc, c) => acc + c.percentage, 0) / totalCandidatesCount) * 10
        ) / 10
      : 0;

  const shortageCandidates = searchedCandidates.filter((c) => c.percentage < 75);
  const goodStandingCandidates = searchedCandidates.filter((c) => c.percentage >= 75);

  // Filter based on clicked KPI card
  const displayedCandidates = searchedCandidates.filter((c) => {
    if (standingFilter === 'GOOD') return c.percentage >= 75;
    if (standingFilter === 'SHORTAGE') return c.percentage < 75;
    return true;
  });
  ...
  const exportCandidatesReport = (items: CandidateAttendanceItem[]) => {
    ...
    const csvRows: any[][] = [
      ...
      ['Filter Applied', standingFilter === 'SHORTAGE' ? 'Shortage Risk (<75%)' : standingFilter === 'GOOD' ? 'Good Standing (>=75%)' : 'All Candidates'],
    ...
    link.href = url;
    link.setAttribute('download', `SREC_Candidates_${standingFilter}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Click on &ldquo;Shortage Risk&rdquo; or &ldquo;Good Standing&rdquo; to instantly filter the students.
          </p>
        </div>

        <button
          type="button"
          onClick={() => exportCandidatesReport(displayedCandidates)}
          ...
          title="Export displayed candidate list to Excel"
        >
          <FileSpreadsheet className="w-4 h-4 text-emerald-300" />
          <span>Export {standingFilter !== 'ALL' ? (standingFilter === 'SHORTAGE' ? 'Shortage' : 'Good Standing') : 'All'} Candidates ({displayedCandidates.length})</span>
        </button>
      </div>

      {/* Interactive Clickable KPI Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        {/* Total Candidates Button */}
        <button
          type="button"
          onClick={() => setStandingFilter('ALL')}
          className={`p-5 rounded-2xl border text-left transition-all cursor-pointer ${
            standingFilter === 'ALL'
              ? 'bg-blue-50/80 border-blue-400 ring-2 ring-blue-600 shadow-sm'
        ...
        >
          ...
          <div className="text-2xl font-black text-slate-900 mt-1">{totalCandidatesCount}</div>
          <div className={`text-[11px] font-semibold mt-1 flex items-center gap-1 ${
            standingFilter === 'ALL' ? 'text-blue-700' : 'text-slate-500'
          }`}>
            {standingFilter === 'ALL' ? (
              <span>✓ Currently viewing all</span>
            ) : (
              <span className="underline">Click to show all candidates</span>
            )}
          </div>
        </button>

        {/* Average Attendance Info Card */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-1">
          <div className="text-xs font-bold text-slate-500 uppercase flex items-center justify-between">
            <span>Average Attendance</span>
            <CalendarCheck className="w-4 h-4 text-emerald-600" />
          </div>
          <div className="text-2xl font-black text-emerald-700 mt-1">{avgAttendance}%</div>
          <div className="text-[11px] text-slate-500 mt-1">Autonomous threshold: 75% minimum</div>
        </div>

        {/* Good Standing Clickable Button */}
        <button
          type="button"
          onClick={() => setStandingFilter(standingFilter === 'GOOD' ? 'ALL' : 'GOOD')}
          className={`p-5 rounded-2xl border text-left transition-all cursor-pointer ${
            standingFilter === 'GOOD'
              ? 'bg-emerald-50 border-emerald-500 ring-2 ring-emerald-600 shadow-sm'
              : 'bg-white border-slate-200 hover:border-emerald-400 hover:shadow-xs'
          }`}
          title="Click to view students with >= 75% attendance"
        >
          <div className="text-xs font-bold text-slate-500 uppercase flex items-center justify-between">
            <span>Good Standing (&ge;75%)</span>
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
          </div>
          <div className="text-2xl font-black text-emerald-600 mt-1">{goodStandingCandidates.length}</div>
          <div className={`text-[11px] font-semibold mt-1 flex items-center gap-1 ${
            standingFilter === 'GOOD' ? 'text-emerald-800' : 'text-emerald-600'
          }`}>
            {standingFilter === 'GOOD' ? (
              <span>✓ Filter Active (Click to reset)</span>
            ) : (
              <span className="underline">Click to view eligible students</span>
            )}
          </div>
        </button>

        {/* Shortage Risk Clickable Button */}
        <button
          type="button"
          onClick={() => setStandingFilter(standingFilter === 'SHORTAGE' ? 'ALL' : 'SHORTAGE')}
          className={`p-5 rounded-2xl border text-left transition-all cursor-pointer ${
            standingFilter === 'SHORTAGE'
              ? 'bg-red-50 border-red-500 ring-2 ring-red-600 shadow-sm'
        ...
        >
          ...
          <div className="text-2xl font-black text-red-600 mt-1">{shortageCandidates.length}</div>
          <div className={`text-[11px] font-semibold mt-1 flex items-center gap-1 ${
            standingFilter === 'SHORTAGE' ? 'text-red-800' : 'text-red-600'
          }`}>
            {standingFilter === 'SHORTAGE' ? (
              <span>✓ Filter Active (Click to reset)</span>
            ) : (
              <span className="underline">Click to view shortage students</span>
            )}
          </div>
        </button>
      </div>
        ...
        <div className={`p-3.5 rounded-xl border flex items-center justify-between text-xs animate-in fade-in ${
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">
              Showing {displayedCandidates.length} students in{' '}
              <strong>{standingFilter === 'SHORTAGE' ? 'Shortage Risk (< 75%)' : 'Good Standing (≥ 75%)'}</strong>
            </span>
          </div>
          ...
          >
            ...
            <span>Reset / Show All Students</span>
          </button>
        </div>
      )}
              ...
              {displayedCandidates.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-400 text-xs">
                    No candidates found for the selected {standingFilter !== 'ALL' ? standingFilter.toLowerCase() : ''} filter.
                  </td>
                </tr>
              ) : (
